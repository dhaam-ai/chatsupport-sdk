> **Archived — this documents v1, the `@chat-service/sdk` package.**
>
> It was this repository's root README until the v2 monorepo replaced it, and it
> is kept verbatim below because v1 is deliberately retained as the UI reference
> for v2 (see `docs/MIGRATION-v1-to-v2.md`). Nothing here describes the
> `@dhaam-ccrm/*` packages: the config shape, the package name, and the
> Cognito-token flow are all v1's. For v2, start at the root README.
