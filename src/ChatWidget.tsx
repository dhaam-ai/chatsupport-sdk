      ...
      {showEndChat && onEndChat && (
        <button
          ...
          style={{
            ...
            color: theme.headerText, cursor: 'pointer', padding: '5px 10px',
            borderRadius: '20px', display: 'flex', alignItems: 'center', gap: '4px',
            fontSize: '11px', fontWeight: 600, marginRight: '6px',
            transition: 'all 0.15s', letterSpacing: '0.03em', lineHeight: 1,
          }}
          onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(239,68,68,0.35)'; (e.currentTarget as HTMLButtonElement).style.borderColor = 'rgba(239,68,68,0.6)'; }}
          onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.15)'; (e.currentTarget as HTMLButtonElement).style.borderColor = 'rgba(255,255,255,0.3)'; }}
        >
          <EndChatIcon />
          End
        </button>
...
export function ChatContent({ onClose, styles, config, theme, onStartNewChat }: {
  ...
  const allMessages = React.useMemo(() => {
    const seen = new Set<string>();
    // Dedup key for non-TEXT messages: server sometimes delivers the same media
    // message twice with different IDs (e.g. WS echo + REST history mismatch).
    // Key = senderType + messageType + rounded-second timestamp + content prefix.
    const seenMediaKey = new Set<string>();
    const makeMediaKey = (m: ChatMessage) => {
      const ts = Math.round(new Date(m.timestamp).getTime() / 2000); // 2s bucket
      return `${m.senderType}|${m.messageType}|${ts}|${m.content.slice(0, 40)}`;
    };
    const result: ChatMessage[] = [];
    for (const m of state.messages) {
      if (seen.has(m.id)) continue;
      if (m.messageType !== 'TEXT' && m.messageType !== 'SYSTEM') {
        const key = makeMediaKey(m);
        if (seenMediaKey.has(key)) continue; // drop duplicate media message
        seenMediaKey.add(key);
      }
      seen.add(m.id);
      result.push(m);
    }
