// decodeWaveform — turn a recorded blob into the handful of bar heights a
// voice-note bubble draws.
//
// ---------------------------------------------------------------------------
// Why this resolves a status instead of rejecting
// ---------------------------------------------------------------------------
//
...
// throw at a render tree — a voice note whose waveform failed should still
// play through an `<audio>` element, which has its own decoder and often
// succeeds where `decodeAudioData` did not. So every failure resolves to
// `status: 'failed'` and the caller draws a flat bar.
//
// ---------------------------------------------------------------------------
// Closing the AudioContext is the whole reason this is not two lines
// ---------------------------------------------------------------------------
//
...
// the document. Decoding one context per voice note and never closing it means
// a conversation with forty voice notes stops being able to construct an
// AudioContext at all, at which point playback and the recorder's own
// amplitude meter both break with an error that names nothing to do with
// waveforms. The `finally` below is load-bearing.
...
export type AudioWaveformErrorCode = 'unsupported' | 'read-failed' | 'decode-failed';
...
export interface DecodeWaveformOptions {
  /** Number of bars. Defaults to {@link DEFAULT_WAVEFORM_BUCKETS}; clamped to 1..512. */
  buckets?: number;
}

/** The `idle` result, shared so a binding can compare identity rather than fields. */
export const IDLE_WAVEFORM: WaveformResult = Object.freeze({
...
/** The `loading` result. Same reasoning as {@link IDLE_WAVEFORM}. */
export const LOADING_WAVEFORM: WaveformResult = Object.freeze({
...
function failed(errorCode: AudioWaveformErrorCode): WaveformResult {
  return { status: 'failed', peaks: [], durationMs: null, errorCode };
}
 ...
 * but it is genuinely absent in two environments this has to work in: Safari
 * before 14 (which still receives `MediaRecorder`-produced voice notes from
 * other clients), and jsdom, which is what the test suite runs on. The
...
/**
 * Decodes `blob` to render-ready peaks. Never rejects.
 *
 * SSR-safe: every browser global is read inside the call, so importing this
 * module on a server touches nothing.
 *
 * @param blob The recorded audio.
 * @param options.buckets Number of bars. See {@link DEFAULT_WAVEFORM_BUCKETS}.
 */
export async function decodeWaveform(blob: Blob, options: DecodeWaveformOptions = {}): Promise<WaveformResult> {
  const buckets = options.buckets ?? DEFAULT_WAVEFORM_BUCKETS;

  const AudioContextCtor = getAudioContextCtor();
  if (AudioContextCtor === undefined) return failed('unsupported');
  ...
  try {
    ...
    // Runs on every path, including the early returns above — which is
    // exactly why the close lives here and not after the try/catch.
