...
- 96ababf: Add `@dhaam-ccrm/angular`, the Angular binding for `@dhaam-ccrm/core`.
  ...
  **signals**, plus the §6.5 event catalog with `DestroyRef`-driven teardown.
  ...
  - **Signals, not Observables.** Core's store is a synchronous, reference-stable
    ...
    _is_ the selector cache PRD §6.4 makes every binding write. An RxJS codebase
...
- `@dhaam-ccrm/core` is a **peer dependency**, not a bundled one. The barrel
  re-exports core's error classes, tick helpers and message-list helpers as
  runtime values, so two copies of core in one tree would hand consumers a
  different `ChatClientConfigError` than the one core throws.
