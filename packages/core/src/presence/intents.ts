...
//
// PRD §7.3 lists six client→server frames this module is responsible for
// producing: `typing.start`, `typing.stop`, `message.markRead`,
// `message.markDelivered`, `presence.set`, and `presence.query`. This module
// decides *when* each should go out and with what payload; it never sends one.
// It hands an intent to an `IntentSink`, and the transport layer (T7) wraps it
// in the §7.2 envelope — supplying `v`, a fresh ULID `id`, and `ts` — and
...
/**
 * The six §7.3 client→server frame types this module produces.
 *
