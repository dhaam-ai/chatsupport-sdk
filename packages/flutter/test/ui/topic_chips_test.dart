...
void main() {
  testWidgets('renders nothing when there are no topics', (tester) async {
    await tester.pumpWidget(_wrap(TopicChips(topics: const [], selected: null, onSelect: (_) {})));
    expect(find.byType(ChoiceChip), findsNothing);
  });

  testWidgets('renders one chip per topic', (tester) async {
    await tester.pumpWidget(_wrap(TopicChips(topics: _topics, selected: null, onSelect: (_) {})));
  ...
  testWidgets('marks only the selected topic\'s chip selected', (tester) async {
    await tester.pumpWidget(_wrap(TopicChips(topics: _topics, selected: _topics[0], onSelect: (_) {})));
  ...
  testWidgets('no topic selected -> every chip reads unselected', (tester) async {
    await tester.pumpWidget(_wrap(TopicChips(topics: _topics, selected: null, onSelect: (_) {})));
    final chips = tester.widgetList<ChoiceChip>(find.byType(ChoiceChip)).toList();
  ...
  testWidgets('tapping a chip calls onSelect with that topic', (tester) async {
    ...
    await tester.pumpWidget(_wrap(TopicChips(topics: _topics, selected: null, onSelect: (t) => tapped = t)));
