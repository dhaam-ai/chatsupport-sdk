...
Widget _wrap(RemoteConfig config) => MaterialApp(home: Scaffold(body: HeroHeader(config: config)));

void main() {
  testWidgets('renders nothing at all when the merchant configured no hero content', (tester) async {
    ...
    // data-empty rule.
    expect(find.descendant(of: find.byType(HeroHeader), matching: find.byType(Container)), findsNothing);
  });

  testWidgets('renders the greeting and sub-greeting', (tester) async {
    await tester.pumpWidget(
      _wrap(testRemoteConfig(header: const HeaderAppearance(greeting: 'Hi there', subGreeting: 'How can we help?'))),
    );
    expect(find.text('Hi there'), findsOneWidget);
    expect(find.text('How can we help?'), findsOneWidget);
  });

  testWidgets('a logo only shows when showLogo is true, even with a URL present', (tester) async {
    const header = HeaderAppearance(showLogo: false, logoUrl: 'https://x.test/logo.png');
    await tester.pumpWidget(_wrap(testRemoteConfig(header: header, greeting: null)));
    // Nothing else configured either, and showLogo is false -> still empty.
    expect(find.descendant(of: find.byType(HeroHeader), matching: find.byType(Container)), findsNothing);
  });

  testWidgets('renders an avatar stack, capped at kMaxHeroAvatars, filtering out unsafe URLs', (tester) async {
    await tester.pumpWidget(
      _wrap(
        testRemoteConfig(
  ...
  testWidgets('the background colour resolves through headerBackgroundColor', (tester) async {
    ...
    final Container container = tester.widget<Container>(
      find.descendant(of: find.byType(HeroHeader), matching: find.byType(Container)).first,
    );
    expect((container.decoration! as BoxDecoration).color, const Color(0xFFFF0000));
  });
}
