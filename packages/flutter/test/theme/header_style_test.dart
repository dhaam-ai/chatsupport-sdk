...
void main() {
  group('headerBackgroundColor', () {
    test('an explicit backgroundColor wins over accent', () {
      final color = headerBackgroundColor(const HeaderAppearance(backgroundColor: '#ff0000'), _accent);
    ...
    test('falls back to accent when the published colour cannot be parsed', () {
      final color = headerBackgroundColor(const HeaderAppearance(backgroundColor: 'not-a-color'), _accent);
      expect(color, _accent);
    });

    test('colorSource has no host-page equivalent to defer to — both values resolve to accent', () {
  ...
  group('headerOverlay', () {
    test('solid paints nothing over the background', () {
      expect(headerOverlay(const HeaderAppearance(background: HeaderBackground.solid)), isNull);
    });
    ...
    test('gradient returns a top-down black wash', () {
      final overlay = headerOverlay(const HeaderAppearance(background: HeaderBackground.gradient));
    ...
    test('gradientStrength scales the wash, clamped and NaN-proofed', () {
      final overlay = headerOverlay(
        const HeaderAppearance(background: HeaderBackground.gradient, gradientStrength: 50),
    ...
    test('image with a safe URL returns the overlay with a default 45% scrim', () {
      final overlay = headerOverlay(
        const HeaderAppearance(background: HeaderBackground.image, backgroundImageUrl: 'https://x.test/bg.png'),
      );
    ...
    test('image with an unsafe or absent URL paints nothing — graceful miss, not a broken image', () {
      expect(
        headerOverlay(const HeaderAppearance(background: HeaderBackground.image, backgroundImageUrl: 'javascript:x')),
        isNull,
      );
      expect(headerOverlay(const HeaderAppearance(background: HeaderBackground.image)), isNull);
    });
  });
}
