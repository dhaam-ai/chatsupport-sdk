...
import 'package:dhaam_chat/dhaam_chat.dart' show ChatClient, ChatTarget;
...
class _HostHomePageState extends State<_HostHomePage> {
  ...
  void _openChat({String? outletId}) {
    Navigator.of(context).push<void>(
      MaterialPageRoute<void>(
        builder: (BuildContext context) => _ChatPanelPage(
          config: widget.config,
          outletId: outletId,
          initialSessionId: outletId == null ? widget.config.sessionId : null,
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: <Widget>[
          ...
          const SizedBox(height: 12),
          OutlinedButton.icon(
            onPressed: widget.config.outletId == null
                ? null
                : () => _openChat(outletId: widget.config.outletId),
            icon: const Icon(Icons.storefront_outlined),
            label: const Text('Open outlet chat'),
          ),
          ...
          _Section(
            ...
            children: <Widget>[
              ...
              _Fact(
                'Configured outlet',
                widget.config.outletId == null
                    ? 'not set ($kOutletIdKey)'
                    : widget.config.outletId!,
              ),
...
class _ChatPanelPageState extends State<_ChatPanelPage> {
  ...
  void initState() {
    ...
    final ChatTarget? target = widget.outletId == null
        ? null
        : ChatTarget.merchantOutlet(widget.outletId!);
    ...
    _cubit = ChatWidgetCubit(
      ...
      initialScreen: widget.outletId == null ? null : ScreenName.conversation,
      sessionId: widget.initialSessionId,
      rest: widget.rest,
      target: target,
      identity: ChatIdentity(
          userId: 'b204f228-295e-457f-af08-6639cb4f3378',
          profile: const ChatParticipantProfile()),
      ...
      // Passing `rest` above is enough for the SDK to derive its REST-backed
      // issue reporter, attachment uploader, target-session open/resume and
      // first history page loader.
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ChatWidget(
      ...
      chime: _chime,
      cubit: _cubit,
      showBottomNav: widget.outletId == null,
      showHomeTab: widget.outletId == null,
      onClose: () {
