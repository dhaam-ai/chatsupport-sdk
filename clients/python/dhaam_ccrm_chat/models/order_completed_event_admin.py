import datetime
from collections.abc import Mapping
from typing import (
    Any,
    Literal,
    TypeVar,
    Union,
    cast,
)

from attrs import define as _attrs_define
from dateutil.parser import isoparse

from ..types import UNSET, Unset

T = TypeVar("T", bound="OrderCompletedEventAdmin")


@_attrs_define
class OrderCompletedEventAdmin:
    """Admin-path variant of `OrderCompletedEvent` — see that schema for the full behavior. No `customerId`.

    Attributes:
        event_id (str):
        type_ (Literal['order.completed']):
        occurred_at (datetime.datetime):
        order_id (str):
        value (float):
        merchant (Union[Unset, str]):
        category (Union[Unset, str]):
    """

    event_id: str
    type_: Literal["order.completed"]
    occurred_at: datetime.datetime
    order_id: str
    value: float
    merchant: Union[Unset, str] = UNSET
    category: Union[Unset, str] = UNSET

    def to_dict(self) -> dict[str, Any]:
        event_id = self.event_id

        type_ = self.type_

        occurred_at = self.occurred_at.isoformat()

        order_id = self.order_id

        value = self.value

        merchant = self.merchant

        category = self.category

        field_dict: dict[str, Any] = {}

        field_dict.update(
            {
                "eventId": event_id,
                "type": type_,
                "occurredAt": occurred_at,
                "orderId": order_id,
                "value": value,
            }
        )
        if merchant is not UNSET:
            field_dict["merchant"] = merchant
        if category is not UNSET:
            field_dict["category"] = category

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        event_id = d.pop("eventId")

        type_ = cast(Literal["order.completed"], d.pop("type"))
        if type_ != "order.completed":
            raise ValueError(f"type must match const 'order.completed', got '{type_}'")

        occurred_at = isoparse(d.pop("occurredAt"))

        order_id = d.pop("orderId")

        value = d.pop("value")

        merchant = d.pop("merchant", UNSET)

        category = d.pop("category", UNSET)

        order_completed_event_admin = cls(
            event_id=event_id,
            type_=type_,
            occurred_at=occurred_at,
            order_id=order_id,
            value=value,
            merchant=merchant,
            category=category,
        )

        return order_completed_event_admin
